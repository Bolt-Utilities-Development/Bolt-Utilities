...
client.on("guildcreate", guild => {
    ...
    channels.first().send("Thank you for inviting me! /n set me up with the command `!dashboard`")
});

var http = require('http'); http.createServer(function (req, res) { res.write("Bolt Utilities is online and ready to use!"); res.end(); }).listen(process.env.PORT)
...
module.exports = client;
    ...
    if (!mongooseConnectionString) {
      console.warn("[ERR] Mongo DB URL was not found in secrets.");
    } else {
      mongoose.connect(mongooseConnectionString).then(() => console.log('[MONGO] Connected to mongodb!')).catch(() => console.warn("[ERR] Mongo DB was found in secrets, But the URL is invalid."));
    }

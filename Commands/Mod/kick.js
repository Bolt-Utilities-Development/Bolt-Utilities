    ...
    run: async(client, message, args, util) => {
            ...
            */
            ...
            if(!message.member.permissions.has('KICK_MEMBERS')) {
                return util.quickEmbed(client, message, "You do not have permission to kick members");
            }

            let member = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.member;
            if (!member) {
                return util.quickEmbed(client, message, "You must mention a member to kick");
            }
        
            if(member == message.author.id) {
                return util.quickEmbed(client, message, "You cannot kick yourself");
            } 

            if(message.guild.me.roles.highest.position <= member.roles.highest.position) {
                return util.quickEmbed(client, message, "I can't kick this person as they have an equal or higher role than mine")
            }
            ...
            try {
               await member.kick()
               util.quickEmbed(client, message, `${member} has been kicked`)
            } catch (e) {
                return util.quickEmbed(client, message, "There was an error while trying to kick this user")
            }
    }
}